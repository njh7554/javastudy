...
public class MainClass {
	...
	*/
	...
	/* public static void ex04() {
		...
		File dir = new File("C:" + File.separator + "njh");
		if(dir.exists());
			dir.mkdirs();
		
		File file = new File("jh01.txt");
		...
		try{
			...
			fw.write(cbuf);
			fw.write(str);
			System.out.println("jh01.txt 파일이 생성 되었다.");
		}catch(IOException e) {
			e.printStackTrace();
		}finally {
		try {
	...
	// 문제5. 예외가 발생한 경우 예외 메시지와 발생시간을 저장한 C:\storage\log.txt
	...
	public static void ex06() {
		...
		try {
			...
			bw.close();
			br.close();
			// if(from.length() == to.length()) { 복사에 성공했으면 삭제
				// from.deleteOnExit();
	...
	// 문제 . c:\GDJ61\installer\eclipse-jee-2021-33-R-win32-x86_64.zip 파일을
