...
public class MainClass {
	...
	public static void ex01() {
		...
		*/
		...
		// Person 정보를 HashMap으로 만들기  (실무에서 유용하게 씀) ************************************중요  ↓
		Map<String, Object> person = new HashMap<>(); //맵의 타입<>  뒤에 generic(<K,V>)은 생략 가능 ********
		...
		// 수정
		// put(Key, Value)
		person.put("age", 40); //기존 Key값과 동일한 Key를 전달하면 해당 Key값의 Value가 수정된다.
		
		// 삭제
		int age = (int)person.remove("age");//삭제된 Value가 반환된다. Value는 Object 타입으로 캐스팅해야 사용할 수 있다.
		...
		// Value 가져오기   Key를 전달하면 Value를 가져올 수 있다.
		// get(Key)
